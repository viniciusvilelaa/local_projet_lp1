...
void print_Toptable(int x, int m){
    ...
    std::cout << "Filename" << std::setfill (' ') << std::setw(m) << "Language" << std::setw(m) << "Comments" << std::setw(m) << "Blank" << std::setw(m) << "Code" << std::setw(m) << "All" << '\n';
    std::cout << std::setfill ('-') << std::setw(100) << '\n';
}
...
void print_Infos(fileInfo file, int m){
  std::cout << file.fileName
            ...
            << std::setw(m)
            << file.language
            << std::setw(m - 2 + (file.language.size() - 3))
            << file.comentLines
            << std::setw(m - 2 )
            << file.emptyLines 
            << std::setw(m)
